...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(BC%IOR)
      CASE(1,-1)
         UBAR = 0.5_EB*(UU(BC%IIG,BC%JJG,BC%KKG)+UU(BC%IIG-1,BC%JJG,BC%KKG))
      ...
      CASE(2,-2)
         ...
         VBAR = 0.5_EB*(VV(BC%IIG,BC%JJG,BC%KKG)+VV(BC%IIG,BC%JJG-1,BC%KKG))
         WBAR = 0.5_EB*(WW(BC%IIG,BC%JJG,BC%KKG)+WW(BC%IIG,BC%JJG,BC%KKG-1)) - SF%VEL_T(2)*RAMP_FACTOR
         B1%U_TANG = SQRT(UBAR**2+WBAR**2)
      CASE(3,-3)
         UBAR = 0.5_EB*(UU(BC%IIG,BC%JJG,BC%KKG)+UU(BC%IIG-1,BC%JJG,BC%KKG)) - SF%VEL_T(1)*RAMP_FACTOR
         VBAR = 0.5_EB*(VV(BC%IIG,BC%JJG,BC%KKG)+VV(BC%IIG,BC%JJG-1,BC%KKG)) - SF%VEL_T(2)*RAMP_FACTOR
         WBAR = 0.5_EB*(WW(BC%IIG,BC%JJG,BC%KKG)+WW(BC%IIG,BC%JJG,BC%KKG-1))
         B1%U_TANG = SQRT(UBAR**2+VBAR**2)
   END SELECT
   B1%UVEC = (/UBAR,VBAR,WBAR/)
   B1%U_IMPACT = SQRT(2._EB*HP(BC%IIG,BC%JJG,BC%KKG))
...
INTEGER  :: SURF_GEOMETRY,ITMP,I
REAL(EB) :: RE,H_NATURAL,H_FORCED,FRICTION_VELOCITY=0._EB,YPLUS=0._EB,ZSTAR,DN,TMP_FILM,MU_G,K_G,CP_G,&
            TMP_G,R_DROP,RHO_G,ZZ_G(1:N_TRACKED_SPECIES),CONV_LENGTH,GR,RA,NUSSELT_FORCED,NUSSELT_FREE,&
            FILM_FAC,PHI,NVEC(3)
   ...
   ELSE HTC_TYPE_IF
      ...
      HTC_MODEL_SELECT: SELECT CASE(SFX%HEAT_TRANSFER_MODEL)
         ...
         CASE(RAYLEIGH_HTC_MODEL)
            NVEC = BCX%NVEC
            IF (PRESENT(PARTICLE_INDEX_IN)) NVEC=(/0._EB,0._EB,1._EB/)
            CALL GET_SPECIFIC_HEAT(ZZ_G,CP_G,TMP_FILM)
            CALL RAYLEIGH_HEAT_FLUX_MODEL(H_NATURAL,ZSTAR,DN,P1X%TMP_F,TMP_G,K_G,RHO_G,CP_G,MU_G,P1X%UVEC,GVEC,NVEC,P1X%U_IMPACT)
