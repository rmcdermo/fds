...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (SF%CONE_HEAT_FLUX > 0._EB) THEN
         TSI = MIN(T-T_BEGIN+DT, SF%CONE_FLUX_SMOOTHING_WINDOW+DT)
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
   ...
   END SELECT
   IF (SF%HEAT_TRANSFER_MODEL==FLAME_HTC_MODEL) HTC_LIMIT=25._EB*HTC_LIMIT
...
HTC_MODEL_SELECT: SELECT CASE(SFX%HEAT_TRANSFER_MODEL)
   CASE(DEFAULT_HTC_MODEL,FLAME_HTC_MODEL)
      ...
      IF (SFX%HEAT_TRANSFER_MODEL==FLAME_HTC_MODEL .AND. ABS(DELTA_TMP)>TWO_EPSILON_EB) THEN
         IIG=BCX%IIG
         JJG=BCX%JJG
         KKG=BCX%KKG
         HEAT_TRANSFER_COEFFICIENT=HEAT_TRANSFER_COEFFICIENT*MAX(1._EB,(TMP_FLAME(IIG,JJG,KKG)-ONE_DX%TMP_F)/DELTA_TMP)
      ENDIF
