...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (NO_CONVECTION) METHOD_OF_HEAT_TRANSFER
      ...
      ONE_D%Q_RAD_OUT = SIGMA*ONE_D%EMISSIVITY*ONE_D%TMP_F**4
   ...
   CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER  ! Only for WALL cells
      ...
      ONE_D%Q_CON_F = 2._EB*ONE_D%K_G*(ONE_D%TMP_G-ONE_D%TMP_F)*ONE_D%RDN
      ONE_D%Q_RAD_OUT = SIGMA*ONE_D%EMISSIVITY*ONE_D%TMP_F**4
   ...
   CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
      ...
      ELSEIF (SF%TMP_FRONT>0._EB) THEN
         ONE_D%TMP_F = TMP_0(KKG) + EVALUATE_RAMP(TSI,SF%RAMP(TIME_TEMP)%INDEX,TAU=SF%RAMP(TIME_TEMP)%TAU)*(SF%TMP_FRONT-TMP_0(KKG))
      ELSE
   ...
   CASE (CONVECTIVE_FLUX_BC,NET_FLUX_BC) METHOD_OF_HEAT_TRANSFER
      ...
      IF (ABS(ONE_D%T_IGN-T_BEGIN)<= SPACING(ONE_D%T_IGN) .AND. SF%RAMP(TIME_HEAT)%INDEX>=1) THEN
         TSI = T
      ELSE
         TSI = T - ONE_D%T_IGN
      ENDIF
      ...
      IF (SF%SET_H) THEN
         ...
         ONE_D%HEAT_TRANS_COEF = ONE_D%Q_CON_F/(ONE_D%TMP_G-TMP_OTHER+TWO_EPSILON_EB)
      ELSE
            ...
            SPECIES_LOOP_2: DO N=1,N_TOTAL_SCALARS
               SELECT CASE(SIM_MODE)
                  CASE(DNS_MODE,LES_MODE)
                     D_Z_N = D_Z(:,N)
                     CALL INTERPOLATE1D_UNIFORM(LBOUND(D_Z_N,1),D_Z_N,ONE_D%TMP_G,D_Z_G)
                     IF (SIM_MODE==LES_MODE) CALL GET_VISCOSITY(ZZ_GET,MU_DNS_G,ONE_D%TMP_G)
               END SELECT
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FRACTION) METHOD_OF_MASS_TRANSFER
      ...
      IF (ABS(ONE_D%T_IGN-T_BEGIN)<SPACING(ONE_D%T_IGN) .AND. ANY(SF%RAMP(1:N_TRACKED_SPECIES)%INDEX>=1)) THEN
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      SUM_MASSFLUX_LOOP: DO N=1,N_TRACKED_SPECIES
         IF (ABS(SF%MASS_FLUX(N)) > TWO_EPSILON_EB) THEN  ! Use user-specified ramp-up of mass flux
            IF (ABS(ONE_D%T_IGN-T_BEGIN) < SPACING(ONE_D%T_IGN) .AND. SF%RAMP(N)%INDEX>=1) THEN
               IF (PREDICTOR) TSI = T + DT
               IF (CORRECTOR) TSI = T
            ELSE
               IF (PREDICTOR) TSI = T + DT - ONE_D%T_IGN
            ...
            ELSE
               ONE_D%M_DOT_G_PP_ACTUAL(N) = EVALUATE_RAMP(TSI,SF%RAMP(N)%INDEX,TAU=SF%RAMP(N)%TAU)*SF%MASS_FLUX(N)
            ENDIF
...
UNPACK_WALL_PARTICLE: IF (PRESENT(WALL_INDEX)) THEN
   ...
   ELSEIF (WC%BACK_INDEX>0 .AND. WC%BACK_MESH/=NM) THEN
      ...
      ONE_D_BACK => MESHES(WC%BACK_MESH)%BOUNDARY_ONE_D(WC_BACK%OD_INDEX)
      BP_BACK => MESHES(WC%BACK_MESH)%BOUNDARY_PROPS(WC_BACK%BP_INDEX)
...
ELSEIF (PRESENT(THIN_WALL_INDEX)) THEN
   ...
   IF (TW%BACK_INDEX>0 .AND. TW%BACK_MESH==NM) THEN
      ...
      ONE_D_BACK => BOUNDARY_ONE_D(TW_BACK%OD_INDEX)
   ...
   ELSEIF (TW%BACK_INDEX>0 .AND. TW%BACK_MESH/=NM) THEN
      ...
      ONE_D_BACK => MESHES(TW%BACK_MESH)%BOUNDARY_ONE_D(TW_BACK%OD_INDEX)
...
IF (SF%TMP_GAS_FRONT > 0._EB) ONE_D%TMP_G = TMPA + EVALUATE_RAMP(T-T_BEGIN,SF%RAMP(TIME_TGF)%INDEX)*(SF%TMP_GAS_FRONT-TMPA)
...
ONE_D%HEAT_TRANS_COEF = 0._EB
ONE_D%Q_CON_F = 0._EB
IF (SF%INTERNAL_RADIATION) Q_RAD_OUT_OLD = ONE_D%Q_RAD_OUT
...
IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED .AND. .NOT.PRESENT(THIN_WALL_INDEX)) THEN
   ...
   ENDIF
   ONE_D%Q_DOT_G_PP              = ONE_D%Q_DOT_G_PP              + Q_DOT_G_PP*DT_BC_SUB*RDT
   ONE_D%Q_DOT_O2_PP             = ONE_D%Q_DOT_O2_PP             + Q_DOT_O2_PP*DT_BC_SUB*RDT
   ONE_D%M_DOT_G_PP_ADJUST       = ONE_D%M_DOT_G_PP_ADJUST       + ONE_D%AREA_ADJUST*M_DOT_G_PP_ADJUST*DT_BC_SUB*RDT
   ONE_D%M_DOT_G_PP_ACTUAL       = ONE_D%M_DOT_G_PP_ACTUAL       +                   M_DOT_G_PP_ACTUAL*DT_BC_SUB*RDT
   ONE_D%M_DOT_S_PP(1:SF%N_MATL) = ONE_D%M_DOT_S_PP(1:SF%N_MATL) + M_DOT_S_PP(1:SF%N_MATL)*DT_BC_SUB*RDT
ENDIF
...
PYROLYSIS_PREDICTED_IF_2: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
   ...
   REMESH_GRID: IF (CHANGE_THICKNESS) THEN
      ...
      IF (THICKNESS <=TWO_EPSILON_EB) THEN
         ...
         ONE_D%TMP_F        = MIN(TMPMAX,MAX(TMPMIN,TMP_GAS_BACK))
         ONE_D%TMP_B        = MIN(TMPMAX,MAX(TMPMIN,TMP_GAS_BACK))
         ONE_D%Q_CON_F        = 0._EB
         ONE_D%Q_RAD_OUT      = 0._EB         
         ONE_D%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES) = 0._EB
         ONE_D%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES) = 0._EB
...
ELSE
   ...
   RFACB2 = (KODXB-RFACB)/(KODXB+RFACB)
   QDXKB  = (HTCB*TMP_GAS_BACK + Q_WATER_B + Q_RAD_IN_B     + 3._EB*E_WALLB*SIGMA*ONE_D%TMP_B**4         ) / (KODXB+RFACB)
ENDIF
...
IF (.NOT. SF%INTERNAL_RADIATION) &
   ONE_D%Q_RAD_OUT = ONE_D%Q_RAD_OUT + DT_BC_SUB*(ONE_D%TMP_F_OLD**4+2._EB*ONE_D%TMP_F_OLD**3*(ONE_D%TMP_F-ONE_D%TMP_F_OLD))
...
IF (.NOT. SF%INTERNAL_RADIATION) ONE_D%Q_RAD_OUT = ONE_D%Q_RAD_OUT / DT_BC * SIGMA * ONE_D%EMISSIVITY
...
DO I=1,N_TGA
   ...
   IF (I==1 .OR. MOD(I,NINT(1._EB/(TGA_HEATING_RATE*DT_TGA)))==0) THEN
      ...
      WRITE(LU_TGA,TCFORM) REAL(T_TGA,FB), REAL(ONE_D%TMP_F-TMPM,FB), (REAL(SURF_DEN(N)/SURF_DEN_0,FB),N=0,SF%N_MATL), &
                           ...
                           REAL(HRR,FB), REAL(ONE_D%HEAT_TRANS_COEF*(SF%TMP_GAS_FRONT-ONE_D%TMP_F)*0.001_EB/SURF_DEN_0,FB)
   ENDIF
ENDDO
