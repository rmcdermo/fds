...
NAMELIST /COMB/ AIT_EXCLUSION_ZONE,AUTO_IGNITION_TEMPERATURE,CHECK_REALIZABILITY,EXTINCTION_MODEL,&
                ...
                SPEC_CHEM_ID,&
                ODE_SOLVER,RADIATIVE_FRACTION,RICHARDSON_ERROR_TOLERANCE,SPEC_CHEM_CP_FAC,SUPPRESSION,TAU_CHEM,TAU_FLAME
...
NAMELIST /REAC/ A,AIT_EXCLUSION_ZONE,AUTO_IGNITION_TEMPERATURE,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                ...
                RAMP_CFT,RAMP_CHI_R,REAC_ATOM_ERROR,REAC_MASS_ERROR,REVERSE,SOOT_H_FRACTION,SOOT_YIELD,&
                SPEC_ID_CFT,SPEC_ID_N_S,SPEC_ID_NU,THIRD_BODY
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   RN%AUTO_IGNIT_TMP            = AUTO_IGNITION_TEMPERATURE + TMPM ! AIT converted from C to K
   RN%E                         = E*1000._EB
   RN%E_IN                      = E
   ...
   RN%RAMP_CFT                  = RAMP_CFT
   RN%SPEC_ID_CFT               = SPEC_ID_CFT
   RN%REVERSE                   = REVERSE
   ...
   IF (RN%RAMP_CFT/='null')   CALL GET_RAMP_INDEX(RN%RAMP_CFT,  'TIME',RN%RAMP_CFT_INDEX)
...
! Store global auto ignition temperature and check if pilot fuel model is to be used

AUTO_IGNITION_TEMPERATURE = MAXVAL(REACTION%AUTO_IGNIT_TMP)
IF (ANY(ABS(REACTION%AUTO_IGNIT_TMP-AUTO_IGNITION_TEMPERATURE)>TWO_EPSILON_EB)) THEN
   PILOT_FUEL_MODEL=.TRUE.
...
IF (TRIM(SPEC_CHEM_ID)/='null') THEN
   DO NS=1,N_TRACKED_SPECIES
      IF (TRIM(SPEC_CHEM_ID)==TRIM(SPECIES_MIXTURE(NS)%ID)) THEN
         N_SPEC_CHEM = NS
         EXIT
      ENDIF
   ENDDO
   IF (N_SPEC_CHEM < 0) THEN
      WRITE(MESSAGE,'(A,A,A)') 'ERROR: Problem with COMB. SPEC_CHEM_ID=',TRIM(SPEC_CHEM_ID),' not a tracked species.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
ENDIF
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (RN%RAMP_CFT_INDEX > 0) THEN
      DO NS=1,N_TRACKED_SPECIES
         IF (TRIM(RN%SPEC_ID_CFT)==TRIM(SPECIES_MIXTURE(NS)%ID)) THEN
            RN%RAMP_CFT_SPEC_INDEX = NS
            EXIT
         ENDIF
      ENDDO
      WRITE(LU_ERR,*) 'CFT RAMP',RN%RAMP_CFT_INDEX,TRIM(RN%SPEC_ID_CFT),RN%RAMP_CFT_SPEC_INDEX
   ENDIF
...
REAL(EB) :: EMISSIVITY,CONDUCTIVITY,SPECIFIC_HEAT,DENSITY,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE, &
            PEAK_REACTION_RATE,REFRACTIVE_INDEX,POROSITY,MW,&
...
REAL(EB), DIMENSION(MAX_SPECIES,MAX_REACTIONS) :: NU_SPEC,DIFFUSIVITY_SPEC
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,ADJUST_H,ALLOW_SHRINKING,ALLOW_SWELLING,BETA_CHAR,BOILING_TEMPERATURE,&
                ...
                POROSITY,PYROLYSIS_RANGE,REFERENCE_ENTHALPY,REFERENCE_ENTHALPY_TEMPERATURE,&
                REFERENCE_RATE,REFERENCE_TEMPERATURE,REFRACTIVE_INDEX,RGB,&
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   CALL ChkMemErr('READ','ML%DIFFUSIVITY_GAS',IZERO)
      ...
      DO NS=1,MAX_SPECIES
         ...
         DO NS2=1,N_TRACKED_SPECIES
            IF (TRIM(ML%SPEC_ID(NS,NR))==TRIM(SPECIES_MIXTURE(NS2)%ID)) THEN
               ...
               ML%DIFFUSIVITY_GAS(Z_INDEX(NS,NR)) = ML%DIFFUSIVITY_SPEC(NS,NR)
               EXIT
            ENDIF
         ENDDO
...
LOGICAL :: ADIABATIC,BURN_AWAY,FREE_SLIP,NO_SLIP,CONVERT_VOLUME_TO_MASS,IMPERMEABLE,HORIZONTAL
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            DRAG_COEFFICIENT,MINIMUM_BURNOUT_TIME,DELTA_TMP_MAX,BURN_DURATION,CONE_HEAT_FLUX,CONE_FLUX_SMOOTHING_WINDOW,&
...
NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BURN_AWAY,BURN_DURATION,&
                CELL_SIZE_FACTOR,COLOR,CONE_HEAT_FLUX,CONE_FLUX_SMOOTHING_WINDOW,&
                ...
                N_LAYER_CELLS_MAX,NEAR_WALL_EDDY_VISCOSITY,NEAR_WALL_TURBULENCE_MODEL,NET_HEAT_FLUX,NO_SLIP,NPPC,&
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (ANY(MOISTURE_FRACTION>TWO_EPSILON_EB) .OR. &
      ...
      LAYER_LOOP_2: DO NL=1,MAX_LAYERS
         ...
         IF (SURFACE_VOLUME_RATIO(NL)>TWO_EPSILON_EB .AND. THICKNESS(NL)<0._EB) THEN
   ...
   SF%CONE_FLUX_SMOOTHING_WINDOW  = CONE_FLUX_SMOOTHING_WINDOW
   ...
   SELECT CASE(HEAT_TRANSFER_MODEL)
      ...
      CASE('FLAME')
         SF%HEAT_TRANSFER_MODEL = FLAME_HTC_MODEL
         USE_FLAME_HTC_MODEL    = .TRUE.
   END SELECT
...
CONE_FLUX_SMOOTHING_WINDOW     = 10.0
...
NAMELIST /INIT/ BULK_DENSITY_FILE,CELL_CENTERED,CROWN_BASE_HEIGHT,CROWN_BASE_WIDTH,CTRL_ID,DB,DENSITY,DEVC_ID,&
                DIAMETER,DRY,DT_INSERT,DX,DY,DZ,HEIGHT,HRRPUV,ID,MASS_FRACTION,MASS_PER_TIME,MASS_PER_VOLUME,MULT_ID,N_PARTICLES,&
                N_PARTICLES_PER_CELL,NODE_ID,PART_ID,PARTICLE_WEIGHT_FACTOR,PATH_RAMP,RADIATIVE_FRACTION,&
...
NAMELIST /CTRL/  CONSTANT,DELAY,DIFFERENTIAL_GAIN,FUNCTION_TYPE,ID,INITIAL_STATE,&
                 ...
                 SETPOINT,TARGET_VALUE,TRIP_DIRECTION,CONTROL_FORCE
...
NAMELIST /SLCF/ AGL_SLICE,CELL_CENTERED,DB,FACE_CENTERED,FYI,DEBUG,ID,MAXIMUM_VALUE,MATL_ID,MESH_NUMBER,&
