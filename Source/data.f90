...
TYPE OUTPUT_QUANTITY_TYPE
   ...
   LOGICAL :: SLCF_APPROPRIATE=.TRUE.,BNDF_APPROPRIATE=.FALSE.,ISOF_APPROPRIATE=.TRUE.,PART_APPROPRIATE=.FALSE.,&
              ...
              CELL_L_REQUIRED=.FALSE.,HVAC_SMV=.FALSE.
...
OUTPUT_QUANTITY(434:458)%PART_APPROPRIATE = .TRUE.
OUTPUT_QUANTITY(434:458)%SLCF_APPROPRIATE = .FALSE.
OUTPUT_QUANTITY(434:458)%DEVC_APPROPRIATE = .TRUE.
...
OUTPUT_QUANTITY(527)%NAME = 'FLAME VOLUME FRACTION'
OUTPUT_QUANTITY(527)%UNITS = ''
OUTPUT_QUANTITY(527)%SHORT_NAME = 'X_flame'
...
SUBROUTINE THERMO_TABLE_GAS(I_TMP,CP,SPEC_ID,RCON,G_F)
...
SELECT CASE (SPEC_ID)
   ...
   CASE('SOOT')
      TE = MIN(6000._EB,MAX(TE,100._EB))
...
SUBROUTINE THERMO_TABLE_LIQUID(I_TMP,CP,H_V,T_REF,T_MELT,T_BOIL,SPEC_ID,FUEL,DENSITY,MU,K,BETA)
!CP data in this subroutine is taken from 4th Edition of the NIST-JANAF Thermochemical Tables
!The tabulated data in the tables have been curve fit
!For each species CP is first computed in kJ/mol/K and then converted to J/kg/K
CHARACTER(LABEL_LENGTH), INTENT(IN) ::SPEC_ID
...
GAS_NAME_SELECT: SELECT CASE(GAS_NAME)
   ...
   CASE('FORMALDEHYDE')!Methanol as surrogate
      ...
      EPSOK = 481.8_EB
      FORMULA = 'CH2O'
      H_F = -108.580 ! NASA/TP-2002-211556
      IF (RADCAL_NAME=='null') RADCAL_NAME='METHANOL'
   CASE('HELIUM')
      SIGMA = 2.551_EB
      EPSOK = 10.22_EB
...
ELSE
   CALL THERMO_TABLE_GAS(J,CP_TMP,SS%PROP_ID,SS%RCON,G_F_TMP)
ENDIF
