...
REAL(EB), PARAMETER :: C_L_FORCED = 8._EB**(-2._EB/3._EB), C_T_FORCED = C_L_FORCED * 70._EB**(-2._EB/9._EB)
INTEGER, PARAMETER :: NATURAL=1, FORCED=2
REAL(EB), PARAMETER :: TWO_NINETHS=2._EB/9._EB, EIGHT_NINETHS=8._EB/9._EB
...
PR_G = NU_G/ALPHA_G
THETA_NATURAL = 0.5_EB*(TMP_W+TMP_G)*K_G*NU_G*ALPHA_G/(GRAV+TWO_EPSILON_EB)
THETA_FORCED  =     ABS(TMP_W-TMP_G)*K_G*NU_G/(VEL_G+TWO_EPSILON_EB)

IF (THETA_FORCED < THETA_NATURAL**2) THEN
   REGIME = FORCED
   ! see Holman, p. 285, Pr exponent for fully developed turbulent flow
   ! depends on heating or cooling of the fiuid
   IF (TMP_W>TMP_G) THEN
      EXPON_PR = 0.4_EB ! heating fluid
   ELSE
      EXPON_PR = 0.3_EB ! cooling fluid
   ENDIF
ELSE
   REGIME = NATURAL
   ...
   CASE(FORCED) REGIME_SELECT
      FORCED_LOOP: DO ITER=1,MAX_ITER
         ...
         ELSEIF (Z_STAR>Z_L_FORCED .AND. Z_STAR<=Z_T_FORCED) THEN
            NUSSELT = C_L_FORCED * PR_G**TWO_NINETHS * Z_STAR**TWTH
         ELSE
            NUSSELT = C_T_FORCED * PR_G**(EXPON_PR/1.8_EB) * Z_STAR**EIGHT_NINETHS
         ENDIF
