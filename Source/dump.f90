...
REAL(EB) :: Q_CON,VOLSUM,MFT,ZZ_GET(1:N_TRACKED_SPECIES),Y_SPECIES,DEPTH,UN,H_S,RHO_D_DYDN,U_CELL,V_CELL,W_CELL,&
            ...
            PR1,PR2,Z1,Z2,RADIUS,CUT_FACE_AREA,SOLID_PHASE_OUTPUT_CTF,AAA,BBB,CCC,ALP,BET,GAM,MMM,&
            DTMP,TMP_G,RHO_G,VEL_G,MU_G,K_G,CP_G,THETA_FORCED,THETA_NATURAL,D_STAR_FORCED,D_STAR_NATURAL
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(77) ! CONVECTIVE HEAT TRANSFER REGIME
      ...
      IF (SF%HEAT_TRANSFER_MODEL==RAYLEIGH_HTC_MODEL) THEN
         TMP_G = TMP(BC%IIG,BC%JJG,BC%KKG)
         RHO_G = RHO(BC%IIG,BC%JJG,BC%KKG)
         VEL_G = B1%U_TANG
         ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(BC%IIG,BC%JJG,BC%KKG,1:N_TRACKED_SPECIES)
         CALL GET_SPECIFIC_HEAT(ZZ_GET,CP_G,0.5_EB*(B1%TMP_F+TMP_G))
         CALL GET_VISCOSITY(ZZ_GET,MU_G,TMP_G)
         CALL GET_CONDUCTIVITY(ZZ_GET,K_G,TMP_G)
         THETA_NATURAL = 0.5_EB*(B1%TMP_F+TMP_G)*K_G*(K_G/(RHO_G*CP_G))*(MU_G/RHO_G)/(GRAV+TWO_EPSILON_EB)
         THETA_FORCED  =     ABS(B1%TMP_F-TMP_G)*K_G*(K_G/(RHO_G*CP_G))/(VEL_G+TWO_EPSILON_EB)
         Q_CON = ABS(B1%Q_CON_F)
         D_STAR_FORCED  = SQRT(THETA_FORCED/Q_CON)
         D_STAR_NATURAL = (THETA_NATURAL/Q_CON)**0.25_EB
         IF (D_STAR_FORCED < D_STAR_NATURAL) THEN
            SOLID_PHASE_OUTPUT = 1 ! FORCED convection regime
         ELSE
            SOLID_PHASE_OUTPUT = 2 ! NATURAL convection regime
         ENDIF
      ENDIF
