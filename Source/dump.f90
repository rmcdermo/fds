...
IF (HVAC_SOLVE) THEN
   ...
   DO N=1,N_DUCTS
     ...
     ! IF (ALLOCATED(DUCT(N)%HT_INDEX) THEN
      ...
      IF (DUCT(N)%N_WAYPOINTS > 0) WRITE(LU_SMV,'(1X,3F12.5)') (DUCT(N)%WAYPOINT_XYZ(NN,:),NN=1,DUCT(N)%N_WAYPOINTS)
   ENDDO
...
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),ZZ_REAC(1:N_TRACKED_SPECIES),ZZ_PROD(1:N_TRACKED_SPECIES),&
            MU_Z,K_Z,CP_ZN,H_Z, PHI_TILDE,TMP_FLAME_LOC
...
REACTION_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (COMPUTE_ADIABATIC_FLAME_TEMPERATURE .AND. RN%FAST_CHEMISTRY) THEN
      ...
      CALL GET_FLAME_TEMPERATURE(TMP_FLAME_LOC,PHI_TILDE,ZZ_GET,ZZ_REAC,ZZ_PROD,TMPA,NR)
      WRITE(LU_OUTPUT,'(/6X,A,F8.3)') 'Check of equivalence ratio at stoich:   ', PHI_TILDE
      WRITE(LU_OUTPUT,'(6X,A,F8.1)')  'Stoich adiabatic flame temperature (C): ', TMP_FLAME_LOC - TMPM
   ENDIF
...
DO IW=1,N_WALL_CELLS
   ...
   CALL PACK_WALL(NM,IW,WC%SURF_INDEX,1,UNPACK_IT=.FALSE.)
...
DO ITW=1,N_THIN_WALL_CELLS
   ...
   CALL PACK_THIN_WALL(NM,ITW,TW%SURF_INDEX,1,UNPACK_IT=.FALSE.)
   WRITE(LU_CORE(NM)) TW%SURF_INDEX
   WRITE(LU_CORE(NM)) SF%THIN_WALL_STORAGE%REALS,SF%THIN_WALL_STORAGE%INTEGERS,SF%THIN_WALL_STORAGE%LOGICALS
ENDDO
...
USE MEMORY_FUNCTIONS, ONLY: REALLOCATE,PACK_PARTICLE,PACK_WALL,PACK_THIN_WALL,PACK_CFACE
REAL(EB), INTENT(OUT) :: T,DT
INTEGER :: NOM,N,N_T_E_MAX,IP,CLASS_INDEX,IW,ITW,SURF_INDEX,ICF
INTEGER, INTENT(IN) :: NM
LOGICAL :: EX
CHARACTER(MESSAGE_LENGTH) :: MESSAGE
TYPE(OMESH_TYPE), POINTER :: M2=>NULL()
...
DO IW=1,N_WALL_CELLS
   ...
   CALL PACK_WALL(NM,IW,SURF_INDEX,1,UNPACK_IT=.TRUE.)
ENDDO

READ(LU_RESTART(NM)) N_THIN_WALL_CELLS,N_THIN_WALL_CELLS_DIM
IF (ALLOCATED(MESHES(NM)%THIN_WALL)) DEALLOCATE(MESHES(NM)%THIN_WALL) ;
ALLOCATE(MESHES(NM)%THIN_WALL(1:N_THIN_WALL_CELLS_DIM))
DO ITW=1,N_THIN_WALL_CELLS
   READ(LU_RESTART(NM)) SURF_INDEX
   SF => SURFACE(SURF_INDEX)
   READ(LU_RESTART(NM)) SF%THIN_WALL_STORAGE%REALS,SF%THIN_WALL_STORAGE%INTEGERS,SF%THIN_WALL_STORAGE%LOGICALS
   CALL PACK_THIN_WALL(NM,ITW,SURF_INDEX,1,UNPACK_IT=.TRUE.)
ENDDO
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(526)  ! FLAME TEMPERATURE
      ...
      ELSE
         GAS_PHASE_OUTPUT_RES = TMP(II,JJ,KK) - TMPM
      ENDIF
   CASE(527)  ! FLAME VOLUME FRACTION
      IF (TRI_MODEL .OR. USE_FLAME_HTC_MODEL) THEN
         GAS_PHASE_OUTPUT_RES = VOL_FRAC_FLAME(II,JJ,KK)
      ELSE
         GAS_PHASE_OUTPUT_RES = 0._EB
      ENDIF
...
TYPE(BOUNDARY_PROPS_TYPE), POINTER :: BP=>NULL()
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE( 7) ! BURNING RATE
      IF (N_REACTIONS>0) THEN
         ...
         DO NR=1,N_REACTIONS
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT+ONE_D%M_DOT_G_PP_ACTUAL(REACTION(NR)%FUEL_SMIX_INDEX)*ONE_D%AREA_ADJUST
         ENDDO
      ELSE
         SOLID_PHASE_OUTPUT = 0._EB
      ENDIF
   CASE( 8) ! NORMALIZED HEAT RELEASE RATE
      SOLID_PHASE_OUTPUT = 0._EB
      DO NR=1,N_REACTIONS
         SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT+ONE_D%M_DOT_G_PP_ADJUST(REACTION(NR)%FUEL_SMIX_INDEX)*&
                              REACTION(NR)%HEAT_OF_COMBUSTION
      ENDDO
      SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT*0.001_EB/(SURFACE(SURF_INDEX)%SURFACE_DENSITY*ONE_D%AREA_ADJUST)
   CASE( 9) ! HRRPUA
      SOLID_PHASE_OUTPUT = 0._EB
      DO NR=1,N_REACTIONS
         SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT+ONE_D%M_DOT_G_PP_ADJUST(REACTION(NR)%FUEL_SMIX_INDEX)*&
   ...
   CASE(23) ! ADIABATIC SURFACE TEMPERATURE (Ferrari's Method for Solving the Quartic)
      H_W_EFF = 0._EB
      LTMP = 0._EB
      IF ((PRESENT(OPT_WALL_INDEX).OR.PRESENT(OPT_CFACE_INDEX)) .AND. ASSOCIATED(BP)) THEN  ! Look for evaporating liquid
   ...
   CASE(30:32) ! MPUA, CPUA, AMPUA
      ...
      IF (ASSOCIATED(BP)) THEN
         SELECT CASE(INDX)
            CASE(30) ; SOLID_PHASE_OUTPUT = BP%LP_MPUA(LPC%ARRAY_INDEX)
            CASE(31) ; SOLID_PHASE_OUTPUT = BP%LP_CPUA(LPC%ARRAY_INDEX)*0.001_EB
            CASE(32) ; SOLID_PHASE_OUTPUT = BP%A_LP_MPUA(LPC%ARRAY_INDEX)
         END SELECT
      ELSE
         SOLID_PHASE_OUTPUT = 0._EB
      ENDIF
   ...
   CASE(35) ! VISCOUS WALL UNITS (distance from the wall expressed in nondimensional viscous units)
      IF ((PRESENT(OPT_WALL_INDEX).OR.PRESENT(OPT_CFACE_INDEX)) .AND. ASSOCIATED(BP)) THEN
   ...
   CASE(42:44) ! MPUA_Z, CPUA_Z, AMPUA_Z
      ...
      IF (ASSOCIATED(BP)) THEN
         DO NN = 1,N_LAGRANGIAN_CLASSES
            ...
            IF (LPC%LIQUID_DROPLET .AND. LPC%Y_INDEX==Y_INDEX) THEN
               SELECT CASE(INDX)
                  CASE(42) ; SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + BP%LP_MPUA(LPC%ARRAY_INDEX)
                  CASE(43) ; SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + BP%LP_CPUA(LPC%ARRAY_INDEX)*0.001_EB
                  CASE(44) ; SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + BP%A_LP_MPUA(LPC%ARRAY_INDEX)
               END SELECT
            ENDIF
         ENDDO
      ENDIF
      ...
      IF (PRESENT(OPT_WALL_INDEX)) THEN
         ...
         SELECT CASE(BC%IOR)
            CASE( 1)
               ...
               IF (.NOT.CELL(IC2)%SOLID) P2 = RHO(IIG,JJG,KKG)*(H(IIG+1,JJG,KKG)-KRES(IIG+1,JJG,KKG))
            CASE(-1)
               NVEC=(/-1._EB,0._EB,0._EB/)
               Z1 = 0.5_EB*DX(IIG)
               Z2 = DX(IIG)+0.5_EB*DX(IIG-1)
            ...
            CASE( 2)
               ...
               IF (.NOT.CELL(IC2)%SOLID) P2 = RHO(IIG,JJG,KKG)*(H(IIG,JJG+1,KKG)-KRES(IIG,JJG+1,KKG))
            CASE(-2)
               NVEC=(/0._EB,-1._EB,0._EB/)
               Z1 = 0.5_EB*DY(JJG)
               Z2 = DY(JJG)+0.5_EB*DY(JJG-1)
            ...
            CASE( 3)
               ...
               IF (.NOT.CELL(IC2)%SOLID) P2 = RHO(IIG,JJG,KKG)*(H(IIG,JJG,KKG+1)-KRES(IIG,JJG,KKG+1))
            CASE(-3)
               NVEC=(/0._EB,0._EB,-1._EB/)
               Z1 = 0.5_EB*DZ(KKG)
               Z2 = DZ(KKG)+0.5_EB*DZ(KKG-1)
   ...
   CASE(71) ! EFFECTIVE HEAT TRANSFER COEFFICIENT
      DTMP = TMP(BC%IIG,BC%JJG,BC%KKG)-0.5_EB*(ONE_D%TMP_F_OLD+ONE_D%TMP_F)
      IF (ABS(DTMP)>TWO_EPSILON_EB .AND. ABS(ONE_D%Q_CON_F)>TWO_EPSILON_EB) THEN
         SOLID_PHASE_OUTPUT = ONE_D%Q_CON_F/DTMP
      ELSE
         SOLID_PHASE_OUTPUT = ONE_D%HEAT_TRANS_COEF
      ENDIF
...
SELECT CASE(IND)
   ...
   CASE(455)  ! PARTICLE HEAT TRANSFER COEFFICIENT
      PARTICLE_OUTPUT = BOUNDARY_ONE_D(LP%OD_INDEX)%HEAT_TRANS_COEF
   CASE(456)  ! PARTICLE RADIATIVE HEAT FLUX
      PARTICLE_OUTPUT = (BOUNDARY_ONE_D(LP%OD_INDEX)%Q_RAD_IN-BOUNDARY_ONE_D(LP%OD_INDEX)%Q_RAD_OUT)*0.001_EB
   CASE(457)  ! PARTICLE CONVECTIVE HEAT FLUX
      PARTICLE_OUTPUT = BOUNDARY_ONE_D(LP%OD_INDEX)%Q_CON_F*0.001_EB
   CASE(458)  ! PARTICLE TOTAL HEAT FLUX
      PARTICLE_OUTPUT = ( BOUNDARY_ONE_D(LP%OD_INDEX)%Q_RAD_IN-BOUNDARY_ONE_D(LP%OD_INDEX)%Q_RAD_OUT &
                        + BOUNDARY_ONE_D(LP%OD_INDEX)%Q_CON_F )*0.001_EB
