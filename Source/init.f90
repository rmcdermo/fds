...
IF (TRI_MODEL) THEN
   ...
   M%TMP_FLAME=TMPA
   ALLOCATE(M%VOL_FRAC_FLAME(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','VOL_FRAC_FLAME',IZERO)
   M%VOL_FRAC_FLAME=0._EB
ENDIF
...
TYPE (WALL_SAVE_TYPE), SAVE, ALLOCATABLE, DIMENSION(:) :: WALL_SAVE,THIN_WALL_SAVE
...
IF (.NOT.ALLOCATED(WALL_SAVE)) ALLOCATE(WALL_SAVE(NMESHES))

DO NOM=1,NMESHES
   IF (.NOT.ALLOCATED(WALL_SAVE(NOM)%CELL_INDICES)) ALLOCATE(WALL_SAVE(NOM)%CELL_INDICES(WALL_SAVE(NOM)%DIMEN))
   IF (.NOT.ALLOCATED(WALL_SAVE(NOM)%SURF_INDICES)) ALLOCATE(WALL_SAVE(NOM)%SURF_INDICES(WALL_SAVE(NOM)%DIMEN))
ENDDO
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (PROCESS(NOM)/=MY_RANK) THEN
      IF (COUNT(WALL_SAVE(NOM)%CELL_INDICES(WALL_SAVE(NOM)%START:WALL_SAVE(NOM)%COUNTER)==WC%BACK_INDEX)>0) THEN
         ! This back wall index is already on the list, so just change it to the short list index, II
         DO II=WALL_SAVE(NOM)%START,WALL_SAVE(NOM)%COUNTER
            IF (WALL_SAVE(NOM)%CELL_INDICES(II)==WC%BACK_INDEX) WC%BACK_INDEX = II
         ENDDO
         CYCLE
      ENDIF
      IF (WALL_SAVE(NOM)%COUNTER==WALL_SAVE(NOM)%DIMEN) CALL REALLOCATE_WALL_SAVE
...
DO N=1,M%N_NEIGHBORING_MESHES
   ...
   DO IC=1,CELL_COUNT(NOM)
      ...
      DO IOR=-3,3
         IF (IOR==0) CYCLE
         IF (SURFACE(M4%CELL(IC)%SURF_INDEX(IOR))%HT_DIM>1) THEN
            IF (COUNT(WALL_SAVE(NOM)%CELL_INDICES(WALL_SAVE(NOM)%START:WALL_SAVE(NOM)%COUNTER)==&
                M4%CELL(IC)%WALL_INDEX(IOR))>0) CYCLE
            IF (WALL_SAVE(NOM)%COUNTER==WALL_SAVE(NOM)%DIMEN) CALL REALLOCATE_WALL_SAVE
            WALL_SAVE(NOM)%COUNTER = WALL_SAVE(NOM)%COUNTER + 1
            WALL_SAVE(NOM)%CELL_INDICES(WALL_SAVE(NOM)%COUNTER) = M4%CELL(IC)%WALL_INDEX(IOR)
            WALL_SAVE(NOM)%SURF_INDICES(WALL_SAVE(NOM)%COUNTER) = M4%CELL(IC)%SURF_INDEX(IOR)
         ENDIF
      ENDDO
   ENDDO
ENDDO
...
! DEFINITION MESHES(NM)%OMESH(NOM)%WALL_RECV_BUFFER%INDEX(1:N_ITEMS)
...
DO NOM=1,NMESHES
   ...
   OS%START_INDEX = WALL_SAVE(NOM)%START
   IF (WSC==0) CYCLE
   ALLOCATE(OS%ITEM_INDEX(WSC-WSCS+1))
   ALLOCATE(OS%SURF_INDEX(WSC-WSCS+1))
   OS%ITEM_INDEX(1:WSC-WSCS+1) = WALL_SAVE(NOM)%CELL_INDICES(WSCS:WSC)
   OS%SURF_INDEX(1:WSC-WSCS+1) = WALL_SAVE(NOM)%SURF_INDICES(WSCS:WSC)
   WALL_SAVE(NOM)%START = WALL_SAVE(NOM)%COUNTER + 1
ENDDO
...
THIN_WALL_LOOP: DO ITW=1,M%N_THIN_WALL_CELLS
   ...
   FIND_BACK_THIN_WALL_CELL: DO  ! Look for the back wall cell; that is, the wall cell on the other side of the obstruction
      ...
      IF (ITW2>0 .AND. OM%CELL(IC)%THIN_OBST_INDEX(-IOR,IEC)==OB%ORDINAL) THEN ! the back thin wall is found
         TW%BACK_INDEX = OM%CELL(IC)%THIN_WALL_INDEX(-IOR,IEC)
         TW%BACK_MESH  = NOM
...
! Use the recently populated CELL(IC)%THIN_WALL_INDEX(IOR,IEC) and count the number of
! thin wall cells that need to be sent to neighboring meshes.

IF (.NOT.ALLOCATED(THIN_WALL_SAVE)) ALLOCATE(THIN_WALL_SAVE(NMESHES))

DO NOM=1,NMESHES
   IF (.NOT.ALLOCATED(THIN_WALL_SAVE(NOM)%CELL_INDICES)) ALLOCATE(THIN_WALL_SAVE(NOM)%CELL_INDICES(THIN_WALL_SAVE(NOM)%DIMEN))
   IF (.NOT.ALLOCATED(THIN_WALL_SAVE(NOM)%SURF_INDICES)) ALLOCATE(THIN_WALL_SAVE(NOM)%SURF_INDICES(THIN_WALL_SAVE(NOM)%DIMEN))
ENDDO
      ...
      DO IOR=-3,3
         ...
         DO IEC=1,3
            IF (M4%CELL(IC)%THIN_WALL_INDEX(IOR,IEC)>0) THEN
               IF (THIN_WALL_SAVE(NOM)%COUNTER==THIN_WALL_SAVE(NOM)%DIMEN) CALL REALLOCATE_THIN_WALL_SAVE
               THIN_WALL_SAVE(NOM)%COUNTER = THIN_WALL_SAVE(NOM)%COUNTER + 1
               THIN_WALL_SAVE(NOM)%CELL_INDICES(THIN_WALL_SAVE(NOM)%COUNTER) = M4%CELL(IC)%THIN_WALL_INDEX(IOR,IEC)
               THIN_WALL_SAVE(NOM)%SURF_INDICES(THIN_WALL_SAVE(NOM)%COUNTER) = M4%CELL(IC)%THIN_SURF_INDEX(IOR,IEC)
            ENDIF
         ENDDO
      ENDDO
   ENDDO
...
DO NOM=1,NMESHES
   ...
   IF (WSC==0) CYCLE
   M3 => M%OMESH(NOM)
   ALLOCATE(OS%ITEM_INDEX(WSC))
   ALLOCATE(OS%SURF_INDEX(WSC))
   OS%ITEM_INDEX(1:WSC-WSCS+1) = THIN_WALL_SAVE(NOM)%CELL_INDICES(WSCS:WSC)
   OS%SURF_INDEX(1:WSC-WSCS+1) = THIN_WALL_SAVE(NOM)%SURF_INDICES(WSCS:WSC)
ENDDO
...
ALLOCATE(DUMMY(WALL_SAVE(NOM)%DIMEN+100))
DUMMY(1:WALL_SAVE(NOM)%DIMEN) = WALL_SAVE(NOM)%CELL_INDICES(1:WALL_SAVE(NOM)%DIMEN)
CALL MOVE_ALLOC(DUMMY,WALL_SAVE(NOM)%CELL_INDICES)
ALLOCATE(DUMMY(WALL_SAVE(NOM)%DIMEN+100))
DUMMY(1:WALL_SAVE(NOM)%DIMEN) = WALL_SAVE(NOM)%SURF_INDICES(1:WALL_SAVE(NOM)%DIMEN)
CALL MOVE_ALLOC(DUMMY,WALL_SAVE(NOM)%SURF_INDICES)
WALL_SAVE(NOM)%DIMEN = WALL_SAVE(NOM)%DIMEN + 100

END SUBROUTINE REALLOCATE_WALL_SAVE


SUBROUTINE REALLOCATE_THIN_WALL_SAVE

INTEGER, ALLOCATABLE, DIMENSION(:) :: DUMMY

ALLOCATE(DUMMY(THIN_WALL_SAVE(NOM)%DIMEN+100))
DUMMY(1:THIN_WALL_SAVE(NOM)%DIMEN) = THIN_WALL_SAVE(NOM)%CELL_INDICES(1:THIN_WALL_SAVE(NOM)%DIMEN)
CALL MOVE_ALLOC(DUMMY,THIN_WALL_SAVE(NOM)%CELL_INDICES)
ALLOCATE(DUMMY(THIN_WALL_SAVE(NOM)%DIMEN+100))
DUMMY(1:THIN_WALL_SAVE(NOM)%DIMEN) = THIN_WALL_SAVE(NOM)%SURF_INDICES(1:THIN_WALL_SAVE(NOM)%DIMEN)
CALL MOVE_ALLOC(DUMMY,THIN_WALL_SAVE(NOM)%SURF_INDICES)
THIN_WALL_SAVE(NOM)%DIMEN = THIN_WALL_SAVE(NOM)%DIMEN + 100

END SUBROUTINE REALLOCATE_THIN_WALL_SAVE
...
PRIMARY_WALL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   WC => M%WALL(IW)
   SF => SURFACE(WC%SURF_INDEX)
   IF (SF%HT_DIM==1 .OR. WC%BACK_INDEX<=0) CYCLE PRIMARY_WALL_LOOP_1
   IF (SF%NORMAL_DIRECTION_ONLY) CYCLE PRIMARY_WALL_LOOP_1
...
PRIMARY_THIN_WALL_LOOP_1: DO ITW=1,M%N_THIN_WALL_CELLS
   TW => M%THIN_WALL(ITW)
   SF => SURFACE(TW%SURF_INDEX)
   IF (SF%HT_DIM==1 .OR. TW%BACK_INDEX<=0) CYCLE PRIMARY_THIN_WALL_LOOP_1
...
PRIMARY_WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (SF%HT_DIM==1 .OR. WC%BACK_INDEX<=0) CYCLE PRIMARY_WALL_LOOP
   ...
   PRIMARY_NODE_LOOP: DO I=1,NWP
      ...
      OTHER_MESH_LOOP: DO NOM=1,NMESHES
         IF (NM==NOM) CYCLE
         ALTERNATE_WALL_LOOP_2: DO IW2=1,M%OMESH(NOM)%WALL_RECV_BUFFER%N_ITEMS
            CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NOM,WALL_INDEX=IW2)
         ENDDO ALTERNATE_WALL_LOOP_2
         ALTERNATE_WALL_LOOP_2D: DO ITW2=1,M%OMESH(NOM)%THIN_WALL_RECV_BUFFER%N_ITEMS  !  THIN_WALL cells, neighboring meshes
...
PRIMARY_THIN_WALL_LOOP: DO ITW=1,M%N_THIN_WALL_CELLS
   ...
   IF (SF%HT_DIM==1 .OR. TW%BACK_INDEX<=0) CYCLE PRIMARY_THIN_WALL_LOOP
   ...
   PRIMARY_THIN_NODE_LOOP: DO I=1,NWP
      ...
      OTHER_MESH_LOOP_B: DO NOM=1,NMESHES
         IF (NM==NOM) CYCLE
         ALTERNATE_WALL_LOOP_2B: DO IW2=1,M%OMESH(NOM)%WALL_RECV_BUFFER%N_ITEMS  ! WALL cells, neighboring meshes
            CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NOM,WALL_INDEX=IW2)
         ENDDO ALTERNATE_WALL_LOOP_2B
         ALTERNATE_WALL_LOOP_2C: DO ITW2=1,M%OMESH(NOM)%THIN_WALL_RECV_BUFFER%N_ITEMS  !  THIN_WALL cells, neighboring meshes
            CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NOM,THIN_WALL_INDEX=ITW2)
...
IF (REMOVE .AND. SURFACE(WC%SURF_INDEX)%PYROLYSIS_MODEL==PYROLYSIS_SPECIFIED) THEN
   ...
   END SELECT
   IW_OLD = CELL(IC)%WALL_INDEX(-IOR)
   IF (IW_OLD>0) THEN
      B1 => MESHES(NM)%BOUNDARY_PROP1(WC%B1_INDEX)
      IF (MESHES(NM)%WALL(IW_OLD)%SURF_INDEX==WC%SURF_INDEX) B1%T_IGN = &
         MESHES(NM)%BOUNDARY_PROP1(MESHES(NM)%WALL(IW_OLD)%B1_INDEX)%T_IGN
   ENDIF
ENDIF
