...
IF (TRI_MODEL) THEN
   ...
   M%TRI_COR=1._EB
ENDIF
IF (TRI_MODEL .OR. USE_FLAME_HTC_MODEL) THEN
...
DO NOM=1,NMESHES
   ...
   M%OMESH(NOM)%N_WALL_CELLS_RECV = MAX(0,WSC-WSCS+1)
   M%OMESH(NOM)%N_WALL_CELLS_RECV_START = WALL_SAVE(NOM)%START
   IF (WSC==0) CYCLE
   ALLOCATE(M%OMESH(NOM)%WALL_CELL_INDICES_RECV(WSC-WSCS+1))
   ALLOCATE(M%OMESH(NOM)%WALL_SURF_INDICES_RECV(WSC-WSCS+1))
   M%OMESH(NOM)%WALL_CELL_INDICES_RECV(1:WSC-WSCS+1) = WALL_SAVE(NOM)%CELL_INDICES(WSCS:WSC)
   M%OMESH(NOM)%WALL_SURF_INDICES_RECV(1:WSC-WSCS+1) = WALL_SAVE(NOM)%SURF_INDICES(WSCS:WSC)
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   WC => M%WALL(IW)
   SF => SURFACE(WC%SURF_INDEX)
   IF (SF%N_WALL_STORAGE_INTEGERS<1 .OR. SF%HT_DIM>1) THEN
...
DO NOM=1,NMESHES
   ...
   M%OMESH(NOM)%N_THIN_WALL_CELLS_RECV = MAX(0,WSC-WSCS+1)
   M%OMESH(NOM)%N_THIN_WALL_CELLS_RECV_START = THIN_WALL_SAVE(NOM)%START
   IF (WSC==0) CYCLE
   M3 => M%OMESH(NOM)
   ALLOCATE(M3%THIN_WALL_CELL_INDICES_RECV(WSC))
   ALLOCATE(M3%THIN_WALL_SURF_INDICES_RECV(WSC))
   M3%THIN_WALL_CELL_INDICES_RECV(1:WSC-WSCS+1) = THIN_WALL_SAVE(NOM)%CELL_INDICES(WSCS:WSC)
   M3%THIN_WALL_SURF_INDICES_RECV(1:WSC-WSCS+1) = THIN_WALL_SAVE(NOM)%SURF_INDICES(WSCS:WSC)
ENDDO
...
PRIMARY_WALL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.SF%NORMAL_DIRECTION_ONLY) CALL ADJUST_HT3D_ONE_D
...
PRIMARY_WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   PRIMARY_NODE_LOOP: DO I=1,NWP
      ...
      OTHER_MESH_LOOP: DO NOM=1,NMESHES
         ...
         ALTERNATE_WALL_LOOP_2D: DO ITW2=1,M%OMESH(NOM)%N_THIN_WALL_CELLS_RECV  !  THIN_WALL cells, neighboring meshes
...
PRIMARY_THIN_WALL_LOOP: DO ITW=1,M%N_THIN_WALL_CELLS
   ...
   PRIMARY_THIN_NODE_LOOP: DO I=1,NWP
      ...
      OTHER_MESH_LOOP_B: DO NOM=1,NMESHES
         ...
         ALTERNATE_WALL_LOOP_2C: DO ITW2=1,M%OMESH(NOM)%N_THIN_WALL_CELLS_RECV  !  THIN_WALL cells, neighboring meshes
            CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NOM,THIN_WALL_INDEX=ITW2)
