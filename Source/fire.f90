...
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),CP,H_S_N,&
            ...
            TRI_COR_TMP(NUMBER_SPECTRAL_BANDS),TMP_FLAME_TMP,VOL_FRAC_FLAME_TMP
LOGICAL :: Q_EXISTS
...
IF (TRI_MODEL .OR. USE_FLAME_HTC_MODEL) THEN
   TMP_FLAME=TMP
   VOL_FRAC_FLAME=0._EB
ENDIF
IF (TRI_MODEL) TRI_COR=1._EB
   ...
   DO J=1,JBAR
      ILOOP: DO I=1,IBAR
         ...
         CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q(I,J,K),MIX_TIME(I,J,K),CHI_R(I,J,K),&
                                CHEM_SUBIT_TMP,REAC_SOURCE_TERM_TMP,Q_REAC_TMP,TRI_COR_TMP,TMP_FLAME_TMP,VOL_FRAC_FLAME_TMP,&
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TRI_COR_OUT,TMP_FLAME_OUT,VOL_FRAC_FLAME_OUT,TMP_IN,RHO_IN,MU_IN,DELTA,CELL_VOLUME,IIC,JJC,KKC)
...
REAL(EB), INTENT(OUT) :: Q_OUT,MIX_TIME_OUT,CHI_R_OUT,REAC_SOURCE_TERM_OUT(N_TRACKED_SPECIES),Q_REAC_OUT(N_REACTIONS),&
                         TRI_COR_OUT(NUMBER_SPECTRAL_BANDS),TMP_FLAME_OUT,VOL_FRAC_FLAME_OUT
...
REAL(EB) :: ERR_EST,ERR_TOL,A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            TRI_NUMER,TRI_DENOM,TMP_FLAME_LOC,VOL_FRAC_FLAME_LOC,TMP_MIXED,CP_0,CP_FLAME
   ...
   IF (SUPPRESSION) THEN
      ...
      DO NR=1,N_REACTIONS
         ...
         IF (ZZ_0(RN%FUEL_SMIX_INDEX)>TWO_EPSILON_EB .AND. ZZ_0(RN%AIR_SMIX_INDEX)>TWO_EPSILON_EB) THEN
            CALL CHECK_AUTO_IGNITION(EXTINCT,TMP_IN+TMP_SGS,RN%AUTO_IGNITION_TEMPERATURE,IIC,JJC,KKC,NR)
            IF (.NOT.EXTINCT) EXIT
         ENDIF
      ENDDO
   ENDIF
...
TMP_FLAME_IF: IF ((TRI_MODEL .OR. USE_FLAME_HTC_MODEL) .AND. Q_OUT>TWO_EPSILON_EB) THEN

   CALL GET_FLAME_TEMPERATURE(TMP_FLAME_LOC,PHI_TILDE,ZZ_FLAME,ZZ_0,ZZ_MIXED,TMP_IN)

   ! ***** flame volume method ******
   CALL GET_ENTHALPY(ZZ_0,H_IN,TMP_IN)
   CALL GET_ENTHALPY(ZZ_MIXED,H_FLAME,TMP_FLAME_LOC)

   ! fraction of mass in the cell raised to TMP_FLAME_LOC by Q_OUT
   ZETA_FLAME = MAX(0._EB, MIN(1._EB, Q_OUT*DT / (H_FLAME-H_IN) / RHO_IN ))

   ! convert to volume fraction
   CALL GET_MOLECULAR_WEIGHT(ZZ_0,MW_0)
   CALL GET_MOLECULAR_WEIGHT(ZZ_FLAME,MW_FLAME)
   VOL_FRAC_FLAME_LOC = MAX(0._EB, MIN(1._EB, ZETA_FLAME * MW_0/MW_FLAME * TMP_FLAME_LOC/TMP_IN ))
      ...
      DO IBND=1,NUMBER_SPECTRAL_BANDS
         SELECT CASE(TRI_VERSION)
            ...
            CASE(1)
               ...
               CALL GET_SPECIFIC_HEAT(ZZ_0,CP_0,TMP_IN)
               CALL GET_SPECIFIC_HEAT(ZZ_MIXED,CP_FLAME,TMP_FLAME_LOC)
               TMP_MIXED   = ( (1._EB-ZETA_FLAME)*CP_0*TMP_IN + ZETA_FLAME*CP_FLAME*TMP_FLAME_LOC ) / &
                             ( (1._EB-ZETA_FLAME)*CP_0        + ZETA_FLAME*CP_FLAME )
         ...
         TRI_COR_OUT(IBND) = MAX(TWO_EPSILON_EB, MIN(TRI_NUMER/MAX(TRI_DENOM,TWO_EPSILON_EB), TRI_COR_MAX))
      ENDDO
   ENDIF TRI_IF

   TMP_FLAME_OUT = TMP_FLAME_LOC
   VOL_FRAC_FLAME_OUT = VOL_FRAC_FLAME_LOC
...
DO N=-1,1
   DO M=-1,1
      DO L=-1,1
         ...
         IF (.NOT.CELL(CELL_INDEX(IIC+L,JJC+M,KKC+N))%SOLID) TMP_LOC(L,M,N)=TMP(IIC+L,JJC+M,KKC+N)
      ENDDO
   ENDDO
ENDDO
...
!>
!> \param TMP_FLAME_OUT Adiabatic flame temperature in stoichiometric reaction pocket (K)
...
!> \param REAC_INDEX    (Optional) Index of reaction

SUBROUTINE GET_FLAME_TEMPERATURE(TMP_FLAME_OUT,PHI_TILDE,ZZ_HAT,ZZ_0,ZZ_IN,TMP_IN,REAC_INDEX)

USE PHYSICAL_FUNCTIONS, ONLY: GET_ENTHALPY
REAL(EB),INTENT(IN) :: TMP_IN,ZZ_0(1:N_TRACKED_SPECIES),ZZ_IN(1:N_TRACKED_SPECIES)
INTEGER, OPTIONAL, INTENT(IN) :: REAC_INDEX
REAL(EB),INTENT(OUT) :: TMP_FLAME_OUT,ZZ_HAT(1:N_TRACKED_SPECIES),PHI_TILDE
REAL(EB) :: H_0,TMP_1,TMP_2,H_1,H_2,H_REL_ERROR,ZZ_HAT_0(1:N_TRACKED_SPECIES),Z_F_0,Z_F,Z_A_0,Z_A
INTEGER :: NR,NS,ITER,REAC_INDEX_LOC
...
ZZ_HAT = ZZ_IN
PHI_TILDE = 0._EB
Z_F_0=0._EB; Z_F=0._EB
Z_A_0=0._EB; Z_A=0._EB

! Do not assume one reaction; first, find all "fuel" and "air" species
IF (PRESENT(REAC_INDEX)) THEN
   REAC_INDEX_LOC=REAC_INDEX
   RN=>REACTION(REAC_INDEX)
   Z_F_0 = ZZ_0(RN%FUEL_SMIX_INDEX)
   Z_A_0 = ZZ_0(RN%AIR_SMIX_INDEX)
   Z_F = ZZ_IN(RN%FUEL_SMIX_INDEX)
   Z_A = ZZ_IN(RN%AIR_SMIX_INDEX)
ELSE
   REAC_INDEX_LOC=-1
   DO NR=1,N_REACTIONS
      RN=>REACTION(NR)
      Z_F_0 = Z_F_0 + ZZ_0(RN%FUEL_SMIX_INDEX)
      Z_A_0 = Z_A_0 + ZZ_0(RN%AIR_SMIX_INDEX)
      Z_F = Z_F + ZZ_IN(RN%FUEL_SMIX_INDEX)
      Z_A = Z_A + ZZ_IN(RN%AIR_SMIX_INDEX)
   ENDDO
ENDIF

! This construct for the equivalence ratio does not rely on a single reaction

IF (Z_A>TWO_EPSILON_EB) THEN
   ! Excess AIR
   PHI_TILDE = (Z_A_0 - Z_A) / MAX( Z_A_0, TWO_EPSILON_EB )
ELSE
   ! Excess FUEL
   PHI_TILDE = Z_F_0 / MAX( (Z_F_0 - Z_F), TWO_EPSILON_EB )
ENDIF
...
IF (PHI_TILDE<1._EB) THEN
   ! Excess AIR
   DO NS=1,N_TRACKED_SPECIES
      IF ( IS_FUEL(NS,REAC_INDEX_LOC) ) THEN
         ZZ_HAT_0(NS) = ZZ_0(NS)
         ZZ_HAT(NS)   = 0._EB
      ELSEIF ( IS_AIR(NS,REAC_INDEX_LOC) ) THEN
         ZZ_HAT_0(NS) = PHI_TILDE * ZZ_0(NS)
         ZZ_HAT(NS)   = 0._EB
      ELSE  ! Products
         ZZ_HAT_0(NS) = PHI_TILDE * ZZ_0(NS)
...
DO WHILE (ABS(H_REL_ERROR)>ERROR_TOL)
   ...
   IF (ABS(H_2-H_1)>TWO_EPSILON_EB) THEN
      TMP_FLAME_OUT = TMP_1 + (TMP_2-TMP_1)/(H_2-H_1) * (H_0-H_1)
      TMP_FLAME_OUT = MAX(TMPMIN,MIN(TMPMAX_FLAME,TMP_FLAME_OUT))
   ENDIF
...
LOGICAL FUNCTION IS_FUEL(SPEC_INDEX_FUNC,REAC_INDEX_FUNC)
INTEGER, INTENT(IN) :: SPEC_INDEX_FUNC,REAC_INDEX_FUNC
IS_FUEL=.FALSE.
IF (REAC_INDEX_FUNC>0) THEN
   IF (SPEC_INDEX_FUNC==REACTION(REAC_INDEX_FUNC)%FUEL_SMIX_INDEX) IS_FUEL=.TRUE.
ELSE
   IF (ANY(SPEC_INDEX_FUNC==REACTION(:)%FUEL_SMIX_INDEX)) IS_FUEL=.TRUE.
ENDIF
END FUNCTION IS_FUEL

LOGICAL FUNCTION IS_AIR(SPEC_INDEX_FUNC,REAC_INDEX_FUNC)
INTEGER, INTENT(IN) :: SPEC_INDEX_FUNC,REAC_INDEX_FUNC
IS_AIR=.FALSE.
IF (REAC_INDEX_FUNC>0) THEN
   IF (SPEC_INDEX_FUNC==REACTION(REAC_INDEX_FUNC)%AIR_SMIX_INDEX) IS_AIR=.TRUE.
ELSE
   IF (ANY(SPEC_INDEX_FUNC==REACTION(:)%AIR_SMIX_INDEX)) IS_AIR=.TRUE.
ENDIF
END FUNCTION IS_AIR
...
SPEC_LOOP: DO NS = 1, N_TRACKED_SPECIES
   ...
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC=>WALL(IW)
      ONE_D=>BOUNDARY_ONE_D(WC%OD_INDEX)
      IF (WC%BOUNDARY_TYPE /= SOLID_BOUNDARY .OR. ONE_D%NODE_INDEX > 0) CYCLE WALL_LOOP
      ...
      Y_PLUS = RHO_G*MAX(0.0001_EB,BP%U_TAU)*0.5_EB/(ONE_D%RDN*MU_AIR) ! FDS Tech Guide (8.36)
      ...
      IF (Y_GAS > Y_WALL) THEN
         M_DOT = RHO_G*MAX(0.0001_EB,BP%U_TAU)*(Y_GAS-Y_WALL)/Y_S_PLUS
         M_VAP = -MIN((Y_GAS-Y_N)*RHO_G/(RVC*(1._EB-Y_N)),M_DOT*ONE_D%AREA*DT)
      ELSE
         M_DOT = RHO_G*MAX(0.0001_EB,BP%U_TAU)*(Y_WALL-Y_GAS)/Y_S_PLUS
