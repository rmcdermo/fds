   ...
   DO J=1,JBAR
      ILOOP: DO I=1,IBAR
         ...
         CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q(I,J,K),MIX_TIME(I,J,K),CHI_R(I,J,K),&
                                ...
                                LES_FILTER_WIDTH(I,J,K),DX(I)*DY(J)*DZ(K),I,J,K,&
                                TRI_COR_TMP,TMP_FLAME_TMP,VOL_FRAC_FLAME_TMP)
         !***************************************************************************************
   ...
   ICC_LOOP : DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      ...
      JCC_LOOP : DO JCC=1,NCELL
         ...
         CALL COMBUSTION_MODEL( T,DT,ZZ_GET,CUT_CELL(ICC)%Q(JCC),CUT_CELL(ICC)%MIX_TIME(JCC),&
                                ...
                                LES_FILTER_WIDTH(I,J,K),CUT_CELL(ICC)%VOLUME(JCC),I,J,K,&
                                TRI_COR_TMP,TMP_FLAME_TMP,VOL_FRAC_FLAME_TMP)
         !***************************************************************************************
         IF (REAC_SOURCE_CHECK) THEN ! Store special diagnostic quantities
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,MU_IN,DELTA,CELL_VOLUME,IIC,JJC,KKC,TRI_COR_OUT,TMP_FLAME_OUT,VOL_FRAC_FLAME_OUT)
...
INTEGER, PARAMETER :: TV_ITER_MIN=5,FLAME_VOLUME_METHOD=1
...
TMP_FLAME_IF: IF (TRI_MODEL .AND. Q_OUT>TWO_EPSILON_EB) THEN

   CALL GET_FLAME_TEMPERATURE(TMP_FLAME_LOC,PHI_TILDE,ZZ_FLAME,ZZ_0,ZZ_MIXED,TMP_IN)

   ! ***** flame volume method ******

   SELECT CASE(FLAME_VOLUME_METHOD)
      CASE(1)
         ! method 1 (flame volume from unmixed fraction)
         R1=>REACTION(1)
         IF (PHI_TILDE<1._EB) THEN
            ZETA_FLAME = (1._EB-ZETA)*(ZZ_0(R1%FUEL_SMIX_INDEX) + PHI_TILDE*(1._EB-ZZ_0(R1%FUEL_SMIX_INDEX)))
         ELSE
            ZETA_FLAME = (1._EB-ZETA)*(ZZ_0(R1%AIR_SMIX_INDEX) + 1._EB/PHI_TILDE*(1._EB-ZZ_0(R1%AIR_SMIX_INDEX)))
         ENDIF
      CASE(2)
         ! method 2 (flame volume from heat release rate [should be consistent])
