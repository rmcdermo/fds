...
IF (CC_IBM) THEN
   DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      ...
      ENDDO
   ENDDO
   CALL CCREGION_COMBUSTION(T,DT,NM)
ENDIF
...
IF (SUPPRESSION .AND. .NOT.EXTINCT) THEN
   SELECT CASE(EXTINCT_MOD)
      ...
      CASE(EXTINCTION_4); EXTINCT=.FALSE.
   END SELECT
ENDIF
...
TMP_FLAME_IF: IF ((TRI_MODEL .OR. USE_FLAME_HTC_MODEL) .AND. Q_OUT>TWO_EPSILON_EB) THEN
   ...
   CALL GET_FLAME_TEMPERATURE(TMP_FLAME_LOC,PHI_TILDE,ZZ_FLAME,ZZ_0,ZZ_MIXED,TMP_IN,1)
...
IF (PRESENT(IIC)) THEN
   DO IZ=1,MAX_AIT_EXCLUSION_ZONES
      ...
      IF (XC(IIC)>=AIT_EXCLUSION_ZONE(1,IZ) .AND. XC(IIC)<=AIT_EXCLUSION_ZONE(2,IZ) .AND.  &
          YC(JJC)>=AIT_EXCLUSION_ZONE(3,IZ) .AND. YC(JJC)<=AIT_EXCLUSION_ZONE(4,IZ) .AND.  &
          ZC(KKC)>=AIT_EXCLUSION_ZONE(5,IZ) .AND. ZC(KKC)<=AIT_EXCLUSION_ZONE(6,IZ)) RETURN
   ENDDO
ENDIF
...
SUBROUTINE EXTINCT_4(EXTINCT,ZZ_0,ZZ_IN,TMP_IN,REAC_INDEX)
