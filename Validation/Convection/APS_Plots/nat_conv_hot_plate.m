...
for i=1:length(RAYLEIGH_DOWN)
   NUSSELT_DOWN(i)=0.27*RAYLEIGH_DOWN(i)^0.25;  % Holman Table 7.1
   % (note Kevin uses Incropera and DeWitt, 7th, Eq. 9.32, which has a different constant and exponent)
end
for i=1:length(RAYLEIGH_UP)
   % for this regime Holmand and I&D are the same
...
lh=legend(marker_handle,'Correlation; Down','Correlation; Up','Down (S/\Deltax=8)','Down (S/\Deltax=16)','Down (S/\Deltax=32)',...
  'Up (S/\Deltax=8)','Up (S/\Deltax=16)','Up (S/\Deltax=32)','Location','Northwest','Interpreter','TeX');
...
print(gcf,'-dpdf','nat_conv_hot_plate');
